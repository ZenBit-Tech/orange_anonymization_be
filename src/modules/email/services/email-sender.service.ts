...
export class EmailSenderService {
  ...
  async sendMagicLink(email: string, token: string): Promise<void> {
    try {
      ...
      const brandName = this.configService.get<string>('app.emailBrandName') ?? 'De-ID Studio';
      const brandSubtitle =
        this.configService.get<string>('app.emailBrandSubtitle') ?? 'De-ID and Synthesis';
      const magicLinkValidity =
        this.configService.get<string>('app.magicLinkValidityText') ?? '15 minutes';
